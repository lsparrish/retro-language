...
(defparameter *optimize-operations-max* 16)
(defparameter *optimize-operations-min* 4)
...
(defun ngaro ()
  ...
  (unwind-protect
    (prog1 (values)
      ...
      (let ((opttable (make-hash-table))
            (optcodes (make-array *optimize-operations-max* :element-type 'cell))
            (optpos 0))
        (labels ((opt ()
                      (when (> optpos *optimize-operations-min*)
                        (let ((optip (- ip optpos)))
                          (setf (gethash optip opttable)
                                (compile nil `(lambda () ,@(fun)))))a)
                      (setf optpos 0)
                      (values))
                 (add (op)
                      (if (< optops *optimize-operations-max*)
                        (progn (setf (aref optcodes optpos) op)
                               (incf optpos))
                        (progn (opt))))
                 (fun () ()))
          (loop
